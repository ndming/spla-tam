...
class GaussianRenderTabState(RenderTabState):
    ...
    total_gaussian_count: int = 0
    rendered_gaussian_count: int = 0
...
class GaussianViewer(Viewer):
    ...
    def _populate_rendering_tab(self):
        ...
        with self._rendering_folder:
            with server.gui.add_folder("Gaussian Rasterization"):
                ...
                rendered_gaussian_count_number = server.gui.add_number(
                    "Rendered",
                    initial_value=self.render_tab_state.rendered_gaussian_count,
                    disabled=True,
                    hint="Number of Gaussians rendered in the current view"
                )
        ...
        self._rendering_tab_handles.update(
            {
                "total_gaussian_count_number": total_gaussian_count_number,
                "rendered_gaussian_count_number": rendered_gaussian_count_number,
    ...
    def _after_render(self):
        ...
        self._rendering_tab_handles[
            "total_gaussian_count_number"
        ].value = self.render_tab_state.total_gaussian_count
        self._rendering_tab_handles[
            "rendered_gaussian_count_number"
        ].value = self.render_tab_state.rendered_gaussian_count